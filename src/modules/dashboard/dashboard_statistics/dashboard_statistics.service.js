...
import { CANCELLED_ADMISSION_MODEL } from "../../patient/cancelledAdmission.model.js";
...
// }
async getGenderDistribution() {
  const patients = await PATIENT_MODEL.find({}).select("identityDetails");

  const result = { male: 0, female: 0, other: 0 };

  patients.forEach(patient => {
    const gender = (patient.identityDetails?.gender || "").toLowerCase();
    if (gender === "male") result.male += 1;
    else if (gender === "female") result.female += 1;
    else if (gender === "other") result.other += 1;
  });
...
async getPatientAdmittedByTime(type = 'monthly') {
  ...
  if (type === 'daily') {
    ...
    groupId = {
      week: { $week: "$admissionDetails.admissionDate" }, // MongoDB week number (1–53)
      year: { $year: "$admissionDetails.admissionDate" },
      hour: "$admissionDetails.admissionTime"
    };
  } else if (type === 'monthly') {
  ...
  const admissions = await PATIENT_MODEL.aggregate([
    ...
    { $sort: { "_id.year": 1, "_id.week": 1, "_id.month": 1, "_id.day": 1, "_id.hour": 1 } }
  ]);

  const result = {};
  admissions.forEach(adm => {
    let key = "";
    const entry = { time: adm._id.hour, count: adm.count };

    if (type === 'daily') {
      key = `${adm._id.day}-${adm._id.month}-${adm._id.year}`;
    } else if (type === 'weekly') {
      key = `Week-${adm._id.week}-${adm._id.year}`; // e.g., Week-37-2025
    } else if (type === 'monthly') {
...
async getTotalPdfDocuments() {
    ...
    const result = await PATIENT_MODEL.aggregate(pipeline);
